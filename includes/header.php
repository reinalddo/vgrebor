            ...
            <form action="<?php echo htmlspecialchars($loginUrl, ENT_QUOTES, 'UTF-8'); ?>" method="post" class="d-grid gap-4" novalidate>
              <?php if ($authModalInlineMessage !== ''): ?>
                <div class="rounded-3 border px-3 py-2 small" style="border-color:rgba(248,113,113,0.45); background:rgba(127,29,29,0.22); color:#fecaca; box-shadow:0 0 14px rgba(248,113,113,0.08);">
                  ...
                  <?php
                    $authBlockedWaUrl = !empty($authModalState['blocked'])
                      ? store_config_whatsapp_link((string) store_config_get('whatsapp', ''))
                      : '';
                  ?>
                  <?php if ($authBlockedWaUrl !== ''): ?>
                    <a href="<?php echo htmlspecialchars($authBlockedWaUrl, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener"
                       class="btn btn-sm w-100 mt-2 d-inline-flex align-items-center justify-content-center gap-2 fw-bold"
                       style="background:#25D366; color:#fff; border:none; border-radius:0.5rem;">
